...
use crate::mm::{
    translated_va_to_pa, MapPermission, PageTable, VPNRange, VirtAddr,
};
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let us = get_time_us();
    let ts =
        usize::from(translated_va_to_pa(current_user_token(), ts as *const u8)) as *mut TimeVal;
    unsafe {
        *ts = TimeVal {
            sec: us / 1_000_000,
            usec: us % 1_000_000,
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    if offset != 0 || port & 0x7 == 0 || port & !0x7 != 0 {
        return -1;
    }
    ...
    current_task().unwrap().insert_framed_area(
        VirtAddr::from(start),
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    current_task().unwrap().delete_framed_area(
        VirtAddr::from(start),
        VirtAddr::from(start + len),
    );
    0
